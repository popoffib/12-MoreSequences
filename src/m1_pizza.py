...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(105.0, 50.0),
                rg.Point(90.0, 76.0),
                rg.Point(60.0, 76.0),
                rg.Point(45.0, 50.0),
                rg.Point(60.0, 24.0),
                rg.Point(90.0, 24.0)]
    circle = rg.Circle(rg.Point(75, 50), 30)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'Test 4: 5 green on yellow.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 35)
    circle.fill_color = 'yellow'
    draw_points_on_circle(window, circle, 5, 'green')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4: 77 slices, thin (thickness=2) black lines.'
    window = rg.RoseWindow(400, 450, title)
    circle = rg.Circle(rg.Point(200, 250), 200)
    circle.outline_thickness = 4
    circle.fill_color = 'yellow'
    pizza(window, circle, 77, 'black', 2)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # Test 3:
    title = 'POLYGON test 3: 10 segments with medium green lines on a purple circle.'
...
def run_test_fancy_polygon():
    ...
    # Test 4:
    title = 'FANCY POLYGON test 4: 51 white lines on purple circle' + ' hops = 25.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 300)
    circle.fill_color = 'purple'
    fancy_polygon(window, circle, 51, 25, 'white', 3)
    window.close_on_mouse_click()
